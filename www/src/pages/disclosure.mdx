...
We wanted to make this control as easy and as flexible as possible to use. In it's simplest
...
The content will then be adorned with the `hidden` attribute instead.
...
You are able to nest `Disclosure` components. The `Target` will always be controlled by it's
