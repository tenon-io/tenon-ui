...
<Layout>
    <Helmet>
        <title>Tenon-ui | Textarea</title>
    </Helmet>{children}
</Layout>

## Textarea

To create a `<textarea>`, please use the the `TextareaController` in combination 
with the `Textarea` view component.

These components mirror the functionality of the `text input` almost completely.
    ...
    <div class="form-group">
        <div class="field-wrapper">
            <label for="1bf08342-e063-4b6e-8768-1972fa03ee8e">
                Pet description<span aria-hidden="true"
                                    class="required">
                                    &nbsp;( required )
                               </span>
            </label>
...
so that you can use as many `Textarea` components on a page as you want. As well as some `aria=*`
properties.

### The Textarea view component

The `Tenon-UI Textarea` component provides you with an accessible view component. We recommend 
using this view component if you want to ensure that your `<textarea>` fields are accessible.
...
it is **VERY** important to spread the props object from the render function of the `TextareaController`
onto the `Textarea` component. This passes the required props into our view component, that you
would otherwise use when creating your own views.

The reason for this syntax is that you can now also easily set the props of the `Textarea` component. Note
above how the `labelText` for this specific component was set.
...
Every textarea needs a text label. With this prop you provide the text that will be rendered inside
the `<label>` element.
